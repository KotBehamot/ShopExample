...
WORKDIR /src
COPY ["FleetOrders.csproj", "."]
RUN dotnet restore "./FleetOrders.csproj"
COPY . .
WORKDIR "/src/."
RUN dotnet build "./FleetOrders.csproj" -c $BUILD_CONFIGURATION -o /app/build

# Ten etap służy do publikowania projektu usługi do skopiowania do etapu końcowego
FROM build AS publish
ARG BUILD_CONFIGURATION=Release
RUN dotnet publish "./FleetOrders.csproj" -c $BUILD_CONFIGURATION -o /app/publish /p:UseAppHost=false
...
ENTRYPOINT ["dotnet", "FleetOrders.dll"]